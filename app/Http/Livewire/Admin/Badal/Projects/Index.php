...
namespace App\Http\Livewire\Admin\Badal\Projects;
...
class Index extends Component
{
    ...
    {
        $items = CharityProject::with(['trans', 'category', 'category.trans'])->with('trans')->findOrFail($this->mySelected);
        foreach ($items as $item) {
    ...
    public function mount()
    {
        ...
        $this->categories = CategoryProjects::with('trans')->badal()->orderBy('created_at', 'DESC')->get();
    }

    public function render()
    {

        $query = CharityProject::with(['trans', 'category', 'category.trans'])->badal()->orderBy('created_at', 'DESC');
        ...
        if ($this->search_badal_type  != "") {
            $query = $query->whereHas('badalField', function ($q) {
                $q->where('badal_type', $this->search_badal_type);
            });
            $this->resetPage();
        }
        ...
        return view('livewire.admin.badal.projects.index', compact('items', 'links', 'categories'));
    }
}
