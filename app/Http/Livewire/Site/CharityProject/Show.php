...
class Show extends Component
{
    ...
    public function addCardsToCart()
    {
        ...
        foreach ((session()->get('card') ?? []) as $gift) {
            ...
            $item = new Item(
                CharityProject::class,
                $this->project['id'],
                $gift['donationtype']
            );

            $projectTitle = $this->project->trans
                ? optional($this->project->trans->where('locale', app()->getLocale())->first())->title
                : ($this->project['title'] ?? '');

            $generatedCardImage = $gift['image'] ?? null;

            if (!empty($gift['image'])) {

                $templateImage = $gift['image'];

                if (
                    !str_starts_with($templateImage, 'attachments/')
                    && !str_starts_with($templateImage, '/')
                    && !str_contains($templateImage, 'storage/')
                ) {
                    $templateImage = 'attachments/' . ltrim($templateImage, '/');
                }

                $fromName = auth('account')->user()?->donor?->full_name ?? 'فاعل خير';
                ...
                $generatedCardImage = app(GiftCardGenerator::class)->generate([
                    'template_path'    => $templateImage,
            ...
            $info_gift = [
                ...
                'original_image' => $gift['image'] ?? null,

                'sendCopy'      => $gift['sendCopy'],
            ];

            $this->msg = $cart->addItem(
                $item,
                $this->donationQty,
                $gift['donationAmt'],
                $info_gift
