...
class Orders extends Component
{
    ...
    public function updateSelectStatus(){
        if($this->selectedStatus != ""){
            $query = OrderView::where('refer_id', auth('account')->user()->referer->id)->orderBy('created_at', 'desc')->where('status', $this->selectedStatus);
        }
        else{
            $query = OrderView::where('refer_id', auth('account')->user()->referer->id)->orderBy('created_at', 'desc');
        }
        $this->orderCarousels = [];
