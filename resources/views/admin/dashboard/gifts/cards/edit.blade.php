...
@section('title', trans('gifts.edit_card'))
...
<div class="card">
    <div class="card-body">
        ...
        <form action="{{ route('admin.gifts.cards.update', $item->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="row">
                <div class="col-md-9">
                                        ...
                                        <div class="col-12">
                                            ...
                                            <div class="col-sm-12 mt-1">
                                                <select class="form-select form-select-sm select2" name="category_id" required>
                                                    ...
                                                    <option value="{{ $category->id }}" {{ $item->category_id == $category->id ? 'selected' : '' }}> {{ str_repeat('ـــ ', $category->level - 1) }} {{ @$category->trans->where('locale',$current_lang)->first()->title }} </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-12">
                                            ...
                                            <div class="col-sm-12 mt-1">
                                                <select class="form-select form-select-sm select2" multiple name="occasioins[]">
                                                    ...
                                                    <option value="{{ $occasioin->id }}" {{in_array($occasioin->id,  $item->occasioins?->pluck('id')->toArray()?? [] ) ? 'selected' : '' }}> {{ @$occasioin->trans->where('locale',$current_lang)->first()->title }} </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                ...
                                <div class="accordion-body">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-12">
                                            ...
                                            <div class="col-12">
                                                <input class="form-control" type="number" name="price" step="_any" value="{{ $item->price }}">
                                            </div>
                                            @error('price')
                                            <span class="text-danger">{{ $message }}</span>
                                            @enderror
