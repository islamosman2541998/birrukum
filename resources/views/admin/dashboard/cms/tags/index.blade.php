            ...
            <form action="{{route('admin.tag.index')}}" method="get">
                <div class="row mb-3">
                    ...
                    <div class="search-input col-md-2">
                        ...
                        <a class="btn btn-success btn-sm" href="{{route('admin.tag.index')}}" data-hover="{{ trans('button.reset') }}"><i class="bx bx-refresh"></i></a>
                    </div>
                </div>
            </form>
            {{-- End Form search --}}
            ...
            <div class="table-responsive">
                <table id="main-datatable" class="table table-bordered text-center table-striped table-table-success table-hover table-sm" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center">
                                    @if($item->status == 1)
                                    <a href="{{ route('admin.tag.update-status',$item->id) }}" data-hover="@lang('admin.active')" class="btn btn-neutral text-success btn-sm m-1"><i class="bx bxs-check-square"></i></a>
                                    @else
                                    <a href="{{ route('admin.tag.update-status',$item->id) }}" data-hover="@lang('admin.dis_active')" class="btn btn-neutral text-secondary btn-sm m-1"><i class="bx bx-no-entry"></i></a>
                                    @endif

                                    @if($item->feature == 1)
                                    <a href="{{ route('admin.tag.update-featured',$item->id) }}}" data-hover="@lang('admin.feature')" class="btn btn-neutral text-warning btn-sm m-1"><i class="bx bxs-star"></i></a>
                                    @else
                                    <a href="{{ route('admin.tag.update-featured',$item->id) }}" data-hover="@lang('admin.feature')" class="btn btn-neutral text-secondary btn-sm m-1"><i class="bx bxs-star"></i></a>
                                    @endif
                                    <a href="{{ route('admin.tag.show',$item->id) }}" data-hover="@lang('admin.show')" class="btn btn-neutral text-info btn-sm m-1"><i class="bx bxs-show"></i></a>
                                    <a href="{{ route('admin.tag.edit',$item->id) }}" data-hover="@lang('admin.edit')" class="btn btn-neutral text-primary btn-sm m-1"><i class="bx bxs-edit"></i></a>
                        ...
                        @include('admin.layouts.delete', ['route'=> 'admin.tag.destroy'])
                        @endforeach
                    </tbody>
                </table>
            </div>
