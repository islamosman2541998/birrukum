...
@section('title', trans('attribute.attributeValue'))
@section('title_page', trans('attribute.attributeValue'))
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body  search-group">
                        <div class="row">
                            <div class="col-md-12 text-end">
                                <a href="{{ route('admin.eccommerce.attributes.add', $attributeset_id) }}"
                                    class="btn btn-outline-success btn-sm">@lang('admin.create')</a>
                            </div>

                        </div>
                        {{-- Start Form Search User Search By name,email,role,status --}}
                        <form action="{{ route('admin.eccommerce.attributes.index',$attributeset_id) }}" method="get">
                            ...
                            <div class="row mt-3">
                                ...
                                <div class="search-input col-md-2">
                                    ...
                                    <a class="btn btn-success btn-sm" href="{{ route('admin.eccommerce.attributes.index',$attributeset_id) }}"
                    ...
                    <div class="card-body table-responsive">
                        <form id="update-pages" action="{{ route('admin.eccommerce.attributes.actions') }}" method="post">
                            @csrf
                        </form>
                        <table id="main-datatable" class="table table-bordered dt-responsive nowrap"
                            ...
                            <thead>
                                ...
                                <th>@lang('attribute.attributeSet')</th>
                                <th>@lang('articles.sort')</th>
                                <th>@lang('attribute.color')</th>
                            ...
                            <tbody>
                                @foreach ($attributes as $key => $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->attributeSet->trans->where('locale', $current_lang)->first()->title }}
                                        </td>
                                        <td>{{ $item->sort }}</td>
                                        <td>{{ $item->color }}</td>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                <a href="{{ route('admin.eccommerce.attributes.edit', $item->id) }}"
                                                    class="btn btn-outline-warning btn-sm m-1"
                                                    data-hover="{{ trans('button.edit') }}"><i
                                                        class="bx bxs-edit"></i></a>
                                                @if ($item->status == 1)
                                                    <a href="{{ route('admin.eccommerce.attributes.update-status', $item->id) }}"
                                    ...
                                    @include('admin.layouts.delete', ['route'=> 'admin.eccommerce.attributes.destroy'])
