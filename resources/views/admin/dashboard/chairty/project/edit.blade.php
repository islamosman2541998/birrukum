...
@section('button_page')
    <a href="{{ route('admin.charity.projects.index') }}" class="btn btn-outline-primary waves-effect waves-light ml-3 btn-sm">@lang('button.cancel')</a>
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.charity.projects.update', $project->id) }}" method="post" enctype="multipart/form-data" id="form-submit">
                @csrf
                @method('put')
                <div class="row">
                    <div class="col-md-8">
                                    ...
                                    <div class="accordion-body">
                                        ...
                                        <div class="mt-3 d-flex ">
                                            <div class="form-check form-switch form-check-success">
                                                <input class="form-check-input {{ empty($errors->first('recurring')) ?: 'has-error' }}" type="checkbox" role="switch" name="recurring" {{ $project->recurring  ? 'checked' : '' }} id="recurring">
                                                @if ($errors->has('recurring'))
                                                    <span class="missiong-spam">{{ $errors->first('recurring') }}</span>
                                                @endif
                                            </div>
                                            <label class="form-check-label fw-bold" for="recurring">@lang('Monthly deduction project')</label>
                                        </div>
                                        {{-- number   ------------------------------------------------------------------------------------- --}}
                                        <div class="row mb-3">
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-12">
                                                <select name="categories[]" class="form-select form-select-sm select2 {{ empty($errors->first('categories')) ?: 'has-error' }}" multiple>
                                                    @foreach ($categories as $item)
                                                        <option value="{{ $item->id }}" {{ in_array($item->id, $project->categories->pluck('id')->toArray() ?? []) ? 'selected' : '' }}>
                                            ...
                                            <div class="col-sm-12">
                                                <select class="form-select form-select-sm select2 {{ empty($errors->first('tags')) ?: 'has-error' }}" multiple name="tags[]" aria-label=".form-select-sm example">
                                                    @forelse ($tags as $key => $item)
                                                        <option value="{{ $item->id }}" {{ in_array($item->id, $project->tags->pluck('tag_id')->toArray()) ? 'selected' : '' }}>
                                                            {{ $item->trans->where('locale', $current_lang)->first()->title }}
                                                        </option>
                                                    @empty
                                                    @endforelse
                                    ...
                                    <div class="accordion-body">
                                        ...
                                        <div class="m-auto row ">
                                            ...
                                            <div class="col-sm-6 mt-3">
                                                ...
                                                <div class="form-check form-switch form-check-success">
                                                    <input class="form-check-input {{ empty($errors->first('fast_donation')) ?: 'has-error' }}" type="checkbox" role="switch" name="fast_donation" {{ $project->fast_donation == 1 ? 'checked' : '' }} id="flexSwitchCheckSuccessfast_donation">
                                                    @if ($errors->has('fast_donation'))
                                                        <span class="missiong-spam">{{ $errors->first('fast_donation') }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="accordion-body">
                                            ...
                                            <div class="col-sm-12">
                                                <select class="form-select form-select-sm" name="donation_type" aria-label=".form-select-sm example" id="denation-type">
                                                    ...
                                                    @foreach (App\Enums\DonationTypeEnum::values() as $key => $value)
                                                        <option value="{{ $value }}" {{ $value == @$donation['type'] ? 'selected' : '' }}>
                                        ...
                                        @foreach ($languages as $key => $locale)
                                            <div class="mb-3 row">
                                                ...
                                                <div class="col-sm-12">
                                                    <input class="form-control  {{ empty($errors->first('unit_price')) ?: 'has-error' }}" type="text" name="{{ $locale }}[unit_price]" value="{{ @$project->trans->where('locale', $locale)->first()->unit_price }}"
                                                        id="unit_price{{ $key }}">
                                                </div>
                                                ...
                                                <div class="col-sm-10">
                                                    <div class="form-check">
                                                        @foreach ($payments as $key => $item)
                                                            <div class="col-md-12">
                                                                <input class="form-check-input" type="checkbox" value="{{ $item->id }}" id="flexCheckDefault{{ $key }}" name="payment_method[]"
                    ...
                    <div class="mb-3 row text-end">
                        <div>
                            <a href="{{ route('admin.projects.index') }}" id="chiledBack" class="ml-3 btn btn-outline-primary waves-effect waves-light btn-sm">@lang('button.cancel')</a>
