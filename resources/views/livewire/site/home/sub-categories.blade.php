                    ...
                    <ul class="navbar-nav">
                        @if ($subcategories->count() > 1)
                            <li class="nav-item" wire:click="selectSubcategory(0)">
                                <a class="nav-link {{ $selectedSubcategory == 0 ? 'active' : null }}" aria-current="page">
                                    @lang('All') </a>
                            </li>
                        @endif
                        @if ($subcategories && $subcategories->count() <= 3)
                            @forelse ($subcategories as $subcategory)
                                <li class="nav-item" wire:click="selectSubcategory({{ $subcategory->id }})">
                                    <a class="nav-link {{ $selectedSubcategory == $subcategory->id ? 'active' : null }} "
                        ...
                        @else
                            ...
                            <li class="nav-item" wire:click="selectSubcategory({{ $subcategories[0]->id }})">
                                <a class="nav-link {{ $selectedSubcategory == $subcategories[0]->id ? 'active' : null }}"
                                    aria-current="page"
                                    role="button">{{ $subcategories[0]->trans->where('locale', $current_lang)->first()->title }}</a>
                            </li>
                            <li class="nav-item" wire:click="selectSubcategory({{ $subcategories[1]->id }})">
                                <a class="nav-link {{ $selectedSubcategory == $subcategories[1]->id ? 'active' : null }}"
                                    aria-current="page"
                                    role="button">{{ $subcategories[1]->trans->where('locale', $current_lang)->first()->title }}</a>
                            </li>
                            <li class="nav-item" wire:click="selectSubcategory({{ $subcategories[2]->id }})">
                                <a class="nav-link {{ $selectedSubcategory == $subcategories[2]->id ? 'active' : null }}"
                                    aria-current="page"
                                    role="button">{{ $subcategories[2]->trans->where('locale', $current_lang)->first()->title }}</a>
                            </li>
                            <!-- show the reminder indeces in a dropdown -->
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle
                                {{ !in_array($selectedSubcategory, [0, $subcategories[0]->id, $subcategories[1]->id, $subcategories[2]->id]) ? 'active' : null }}"
                                    role="button" id="navbarDropdownMenuLink" role="button" data-bs-toggle="dropdown"
                                    aria-expanded="false">
                                    أخـــــــرى
                                </a>
                                <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                    @forelse ($subcategories as $key => $subcategory)
                                        @if ($key < 3)
                                            @continue
                                        @endif <!-- skip index 0, 1, 2 -->
                                        <li wire:click="selectSubcategory({{ $subcategory->id }})">
                                            <a class="dropdown-item {{ $selectedSubcategory == $subcategory->id ? 'active' : null }} "
                                                {{ $selectedSubcategory == $subcategory->id ? 'style=color:white!important;' : null }}
                                                role="button">
                                                {{ $subcategory->trans->where('locale', $current_lang)->first()->title }}</a>
                                        </li>
                                    @empty
                                    @endforelse
                                </ul>
