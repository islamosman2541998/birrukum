<div class="col-md-4">
    <div class="project shadow  projects-home">
        <div class="header d-flex">
            ...
            <ul class="project-social p-0 nav flex-column" data-bs-toggle="collapse" href="#socialShare1{{  $project['id'] }}" role="button" aria-expanded="false" aria-controls="socialShare1{{  $project['id'] }}">
        ...
        <div class="project-details my-2">
            <div class="progress mt-3">
                <div class="progress-bar progress-bar-striped bg-success" role="progressbar" style="width: {{ $progressBar['percent'] }}%" aria-valuenow="37" aria-valuemin="0" aria-valuemax="100">
                ...
                <div class="options my-2">
                    <div class="d-flex flex-row">
                        ...
                        <div class="option-item">
                            <label data-toggle="tooltip" data-placement="top" title="{{ $data['name'] }}" class="{{ $colorsAmount[$key % count($colorsAmount)] }} rounded-6 d-flex flex-row gap-1 {{ $unitValueRadio == json_encode($data) ? 'active' : null }}"
            ...
            @case('share')
                <div class="options my-2">
                    @forelse (@$donation['data']??[] as $key => $data)
                    <div class="option-item">
                        <label data-toggle="tooltip" data-placement="top" title="{{ $data['name'] }}" class="{{ $colorsAmount[$key % count($colorsAmount)] }} rounded-6 d-flex flex-row gap-1 {{ $shareValue == json_encode($data) ? 'active' : null }}"
                            style="background-color: {{  $colors[$key % count($colors)] }}!important">
            ...
            <div class="donation-now">
                <input type="فثءف" class="form-control" disabled value="{{  @$donationAmt }} {{ $donationAmt > 0? trans('SAR'):''}}" />
                <a href="{{ route('site.charity-project.show', $project['slug']) }}?amount={{ @$donationAmt }}" class="bg-main btn ">
                    @lang('Donate Now')
                </a>
