            ...
            <table class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <i class="icofont-file-alt text-white btn btn-info btn-sm" wire:click="updateShowNote({{ $details['id'] }})" data-bs-toggle="modal" data-bs-target="#staticBackdrop{{ $details['id'] }}"></i>
                           @if(@$showNote[$details['id']])
                            <div class="modal show" id="staticBackdrop{{ $details['id'] }}" data-bs-keyboard="false" tabindex="-1" aria-hidden="true" style="display: block;">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="staticBackdropLabel{{ $details['id'] }}"> تعليق </h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" wire:click="updateHideNote({{ $details['id'] }})"></button>
                                        </div>
                                        <div class="modal-body" >
                                            <textarea wire:model.defer="notes.{{ $details['id'] }}" class="form-control" cols="30" rows="10">{{ $details['note'] }} </textarea>
                                            <button class="btn btn-sm btn-info mt-3" wire:click="updateComment({{ $details['id'] }} )">@lang('Save')</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="modal fade" id="review_{{ $key }}" tabindex="-1" aria-labelledby="exampleModalReview" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                           <p> <span class="star-rating"> {{ str_repeat('★', @$details['rate']) }} </span></p>
                                           <p> التعليق :  {{ @$details['review'] }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        {{-- <td class="text-center">
                            @if(@$details['giver']['message'])
                                <i type="button" class="icofont-ui-message text-success" data-bs-toggle="modal" data-bs-target="#message_show_{{ $key }}">
                        </i>
                        @endif

                        <!-- Modal -->
                        <div class="modal fade" id="message_show_{{ $key }}" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h6 class="modal-title" id="staticBackdropLabel"> @lang('settings.giver_message') </h6>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        {{ @$details['giver']['message'] }}
                                    </div>
                        ...
                        <td>
                            <select class="form-control" wire:change="changeShippingStatus({{ $details['id'] }}, $event.target.value )">
                                @forelse(App\Enums\ShippingStatusEnum::values() as $status)
                                <option value="{{ $status }}" @if( $details['shipping_status']==$status) selected @endif>
                                    {{ trans( $status) }}
                                </option>
                                @empty
                                @endforelse
