    ...
    <td>
        <div class="d-flex justify-content-center">
            ...
            @can('admin.categories.update-featured')
                @if($item->feature == 1)
                    <a title="@lang('admin.feature')"  class="btn btn-neutral text-warning btn-sm m-1" wire:click="update_featured({{ $item->id }})"><i class="bx bxs-star"></i></a>
                @else
                    <a title="@lang('admin.feature')"  class="btn btn-neutral text-secondary btn-sm m-1" wire:click="update_featured({{ $item->id }})"><i class="bx bx-star"></i></a>
                @endif
            @endcan

            <a  title="@lang('admin.show')" href="{{ route('admin.categories.show', $item->id) }}" class="btn btn-neutral text-info btn-sm m-1"><i class="bx bx-show-alt"></i></a>


            <a title="@lang('admin.edit')" href="{{ route('admin.categories.edit', $item->id) }}"   class="btn btn-neutral text-primary btn-sm m-1"><i class="bx bxs-edit"></i></a>
